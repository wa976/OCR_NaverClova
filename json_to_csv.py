...
output_file_name = "csvfile/ex5.csv"


with open('output/ex5.json', encoding='utf-8') as f:
...
with open(output_file_name, "w", encoding="utf-8", newline="") as output_file:
  ...
  for i in range(len(new_data)):
    if isinstance(new_data[i], str) and new_data[i].isdigit():
      new_data[i] = str(int(new_data[i])).zfill(len(new_data[i]))
  for i in range(len(new_data)):
    write.writerow(new_data[i])
